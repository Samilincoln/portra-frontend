...
export const Route = createFileRoute("/p/$username/$profileSlug")({
  head: ({ params }) => ({
    meta: [
...
function PublicPortfolio() {
  ...
  const profileQ = useQuery({
    queryKey: ["portfolio", username, profileSlug],
    queryFn: () => getPortfolio(username, profileSlug),
    retry: (n, err) => !(err instanceof PortfolioNotFoundError) && n < 2,
  });
  const projectsQ = useQuery({
    queryKey: ["portfolio", username, profileSlug, "projects"],
    queryFn: () => getPortfolioProjects(username, profileSlug),
    enabled: !profileQ.isError,
  });
  const experiencesQ = useQuery({
    queryKey: ["portfolio", username, profileSlug, "experience"],
    queryFn: () => getPortfolioExperiences(username, profileSlug),
    enabled: !profileQ.isError,
  });
  const testimonialsQ = useQuery({
    queryKey: ["portfolio", username, profileSlug, "testimonials"],
    queryFn: () => getPortfolioTestimonials(username, profileSlug),
    enabled: !profileQ.isError,
  });
        ...
        <div className="border-b border-border bg-accent/10">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>
                This is how others see your portfolio.
              </span>
            </div>
            ...
            {projectsQ.isLoading ? (
              ...
              projects.map((p) => (
                <Link
                  ...
                  params={{ username, profileSlug, slug: p.slug }}
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3">
            {blogQ.isLoading ? (
              ...
              latestPosts.map((post) => (
                <BlogCard key={post.id} post={post} username={username} profileSlug={profileSlug} />
              ))
            )}
          </div>
        </div>
...
function BlogCard({ post, username, profileSlug }: { post: BlogPost; username: string; profileSlug: string }) {
  return (
    <Link
...
function ContactForm({ username, profileSlug }: { username: string; profileSlug: string }) {
  const [form, setForm] = useState({ name: "", email: "", message: "" });
  const mut = useMutation({
    mutationFn: () => submitContact(username, profileSlug, form),
    onSuccess: () => {
