...
export async function getExperience(token: string | null, id: string): Promise<Experience> {
...
export async function deleteExperience(token: string | null, id: string): Promise<void> {
  const res = await fetch(`${API_BASE_URL}/api/v1/experiences/${id}`, {
    method: "DELETE",
    headers: authHeaders(token),
  });
...
/**
 * Ask the backend to draft a role description from a short prompt plus
 * whatever role/company/dates are already filled in. Requires a matching
 * `POST /api/v1/experiences/generate-description` endpoint.
 */
export async function generateExperienceDescription(
  ...
  const res = await fetch(`${API_BASE_URL}/api/v1/experiences/generate-description`, {
    method: "POST",
    headers: authHeaders(token),
    body: JSON.stringify(input),
  });
